...
IF memberID <> "" AND len(session("memberID"))<=0 THEN
...
' set rschecktype=getrecordset("select * from assoc_membertypes where id="&session("sa_membertypeid"))
' Photo_Staff_Approval=rschecktype("Photo_Staff_Approval")
'if Photo_Staff_Approval then
conn.execute "update af_members set photo_link='"&session("vendor_logo")&"' where userid="&session("memberID")
'else
' globalconnection.execute "update af_members set photo_link='"&session("vendor_logo")&"',photo_status=1  where userid="&session("sa_id")
' end if
conn.execute "insert into User_Modify_Log (userid,field_modified,previous_value,current_value,modify_date,modify_by) values("&session("memberID")&",'Photo_Link','Null','"&session("vendor_logo")&"', '"&now()&"','"&session("memberID")&"')"
%>
	<script language="JavaScript" type="text/JavaScript">
	self.close();
